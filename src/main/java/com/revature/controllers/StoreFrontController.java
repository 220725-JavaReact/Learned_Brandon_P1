...
public class StoreFrontController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		writer.println("<body><div class=\"user\">"
				...
				+ " Logged In As: " + customer.getUsername() + " " 
				+ "<img src=\"https://cdn-icons-png.flaticon.com/512/8165/8165854.png\" height=30px width=30px/>"
				+ "</h2><div>");
...
//		}
		...
		if(session.getAttribute("storechoice") == null && session.getAttribute("previousorders") == null) {
			writer.print("<table border=\"2\">"
					...
					+ "<tr>"
					+ "<td><form class=\"fixform\" method = \"get\" action = \"/Learned_Brandon_P1/setstore\">"
			...
			writer.write("            </select>\r\n"
					...
					+ "        </form></td>");
			//VIEW ORDERS
		} else if(session.getAttribute("previousorders") != null){
			...
			writer.print(""
					+ "<form method=\"post\" action = \"/Learned_Brandon_P1/addtoorder\">"
					+ "<table border=\"2\">"
					+ "<tr>\r\n"
					+ "    <th colspan=\"4\"><b>" + currentStore.getName() + " Menu</b></th>\r\n"
					+ "  </tr>"
					+ "<tr>\r\n"
					+ "    <th width=\"200\">Product</th>\r\n"
					+ "    <th width=\"90\">Price</th>\r\n"
					+ "    <th width=\"90\">Stock</th>\r\n"
					+ "    <th width=\"130\">Quantity to Add</th>\r\n"
					+ "  </tr>");
			
			for(LineItem lineItem : currentStore.getLineItems()) {
				writer.write("<tr><td>" + lineItem.getProduct().getName() + "</td>"
						+ "<td>$" + lineItem.getProduct().getPrice() + "</td>"
						+ "<td>" + lineItem.getQuantity() + "</td><td width=\"130\">"
						+ "<input type =  \"number\" name=" + lineItem.getProduct().getId() + " size=50 style=\"width: 130px;\" min=0 max=" + lineItem.getQuantity() + " value=0>\r\n"
						+ "</td></tr>");
				
			}
			writer.write("</table>"
					
					+ "<input type = \"submit\" style=\"width: 555px;\" value = \"Add to Order\">"
					+ "</form>");
